...
## 1. 垃圾因子筛除标准 (Hard Discard - 记录但坚决不予优化)

符合以下任意一条规则的因子被判定为 **C 级垃圾因子 (存在致命特征而被平台物理退休的垃圾因子)**。出于审计留痕需要，系统依然会将此类因子保存在本地数据库中进行历史记录（将 `is_garbage` 置为 1 存档），但在后续分析与变体生成流程中将彻底屏蔽不予使用，并触发 WQ 平台的 `Retire (物理退休)` 进行隐藏。
...
flowchart TD
    ...
    C -->|是| D["C级垃圾因子<br/>- 远程 Retire (物理删除)<br/>- 本地 is_garbage=1 隐藏"]
    C -->|否| E{"判定因子分类类型"}
...
## 6. 因子确定性得分与高概率回测过滤法则 (Certainty Score & High-Probability Prioritization)

在处理以“回测任务”为主的大量候选因子源时，**绝不要盲目对所有低夏普或有缺陷的因子进行无差别拯救**。我们必须建立一套科学的「确定性过滤分值」，结合回测阶段的参数优化来极大提高线上通过率。

### 6.1 确定性得分 (Certainty Score) 评估算法
工作台引入了基于因子回测物理指标的确定性得分计算：
*   **基础分 (50分)**：所有未被垃圾拦截的因子起跑线。
*   **正向加分项**：
    *   `Sharpe >= 1.50` (+15分)；`Sharpe >= 1.25` (+5分) — 卓越的预测能力。
    *   `Fitness >= 1.50` (+10分) — 优良的稳定分布。
    *   `Margin >= 20bps (0.0020)` (+10分) — 超额交易空间，能有效抵抗滑点磨损。
    *   `Pasteurization 失败` (+10分) — **此项为加分项**，因为巴斯德过滤失败极易通过 `ts_rank` 分位化完全救活。
*   **负向减分项**：
    *   `自相关性 > 0.85` (-25分) — 共振过于剧烈，正交难度极大。
    *   `自相关性 0.70-0.85` (-10分) — 自相关超标，可通过行业中性化中度拯救。
    *   `换手率过高 / Turnover > 0.70` (-10分) — 调仓过于频繁，交易滑点损耗巨大。
    *   `夏普偏低 / Sharpe < 1.0` (-15分) — 因子基本面逻辑过弱，拯救成功率低。

### 6.2 提升回测端“原始素材”质量的设置建议
要提高优化通过率，必须在**生产端（回测阶段）**进行第一道防线控制，防止垃圾进入数据库：
1.  **近年表现稳定性检验 (Year-by-Year Sharpe Stability)**：
    *   **原则**：拒绝“单年暴利”因子。若因子在 2023 年 Sharpe 为 2.5 但在 2024 年为 -0.3，这属于典型的过拟合特征。
    *   **建议**：在本地分析 PnL 时，提取分年度夏普比率，过滤掉各年度波动过大的因子。
2.  **强制交易活跃度截断 (Active Trading Constraints)**：
    *   **原则**：剔除“僵尸因子”。
    *   **建议**：回测时添加 `(longCount + shortCount) > 250`（或至少 100）的过滤条件，确保因子在目标 Universe 下有广泛的持仓分散度，直接掐死多空归零或单股票占满仓的异常行为。
3.  **横截面与时序扫频协同**：
    *   **原则**：在最初运行回测时，同步提交 Decay 的参数网格（如 Decay=4, 6, 8）进行多维测算。
    *   **建议**：这可以帮助我们在第一步就过滤掉对时序滑点极其脆弱的因子，从而只把高确定性、近年稳定盈利的优质候选导入优化规划中，实现优化通过率的大幅提升。

### 6.3 优化运行的并行加速配置与槽/线程机制 (Parallel Slots & Threads)
在启动大规模因子优化运行任务（生成变体并提交平台回测）时，系统为了兼顾**并发提速**与**规避 429 报错限制**，设计了类似于常规回测任务的「双层并发加速控制体系」：

1. **子任务合并层 (Children Per Request)**：
   * **参数**：`children_per_request` (并发子任务数)。
   * **原理**：将多组优化生成的因子变体表达式，通过合并语法拼接到一个请求中一次性发给 API。例如设置 `children_per_request = 5`，系统会将 5 个变体因子合并，一次 HTTP POST 请求即可同时回测 5 个变体，成倍降低了网络往返开销。
...
2. **Checks 后置评级与 Scheme A 重命名控制 (Deferred Renaming & Grading)**：
   ...
   * **工作流改进**：
     ...
     2. 若本地计算出其自相关超标（判定为 Grade C），则**保留在优化队列中，不判定为垃圾因子，留待工作台通过改写拯救**。只有当因子存在夏普为负、停牌/零交易等无法挽救的问题（属于 C 级中具有致命缺陷的垃圾因子）时，才会自动触发远程物理退休，以节省限额。
     3. 对于未超标且通过自相关初筛的优良因子，正常向 WQ 提交 Checks 校验（任务转换为 `CHECK`）。在这个阶段，系统**不进行细致定级，也绝对不触发重命名**。
     4. 只有当 Checks 校验执行完毕，状态进入 `CHECKED_PASS` 或 `CHECKED_FAIL`，且我们执行 `FETCH` **成功拉取了包含 IS/OS 的 yearly-stats 统计详情后**，系统才会基于完整的样本外数据进行细致的评级并执行相应的改名或优化归类。这确保了因子的命名具有最准确的物理与夏普意义。
