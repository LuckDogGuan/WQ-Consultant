...
符合以下任意一条规则的因子被判定为 **Grade D (垃圾/无潜力因子)**。出于审计留痕需要，系统依然会将此类因子保存在本地数据库中进行历史记录（将 `is_garbage` 置为 1 存档），但在后续分析与变体生成流程中将彻底屏蔽不予使用，并触发 WQ 平台的 `Retire (物理退休)` 进行隐藏。
...
2. **Checks 后置评级与 Scheme A 重命名控制 (Deferred Renaming & Grading)**：
   ...
   * **工作流改进**：
     ...
     2. 若本地计算出其自相关超标（判定为 Grade C），则**保留在优化队列中，不判定为垃圾因子，留待工作台通过改写拯救**。只有当因子存在夏普为负、停牌/零交易等无法挽救的问题（判定为 Grade D）时，才会自动触发远程物理退休，以节省限额。
