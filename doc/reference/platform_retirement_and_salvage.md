# C级垃圾因子平台物理退休与 Check 失败定向挽救机制

## 23. C级垃圾因子平台物理退休与 Check 失败定向挽救机制

本节明确了 Grade C (C级中具有致命缺陷的垃圾因子) 因子在 WorldQuant Brain 平台上的退休隐藏规范，以及针对特定 Check 失败的抢救/优化重写路径，确保本地决策与平台状态实时同步。

### 23.1 平台物理隐藏机制 (DELETE Simulations)

为了维护云端模拟队列的整洁，并防止重复回测或混淆历史记录，被判定为含有致命硬伤的 Grade C 垃圾因子必须从平台“物理退休”：
- **触发条件**：
  1. 在【批量回测】的 `save_children_alphas` 流程中，若子模拟结果的基本指标（如 `sharpe < 0`）触发了 C 级垃圾因子评级。
  2. 在【提交检查】的 `check_worker` 流程中，当平台返回 `FAIL`（如自相关超限、Pasteurization 失败）且最终评估为 C 级垃圾因子时。
- **物理接口**：
...
并非所有被定级为 Grade C（如检查失败或表现缺陷）的因子都是绝对的“死代码”。对于特定类型的平台 Check 失败，系统建立了如下**定向拯救/优化重写模板**：
