...
def detect_flat_pnl(pnl_values: list[Any], threshold: int = FLAT_PNL_THRESHOLD) -> bool:
    """
    检测 PNL 序列末端是否存在"厂字"特征：
    末端连续 >= threshold 个交易日的值完全相同（非 None）。
    """
    if not pnl_values or len(pnl_values) < threshold:
        return False
    try:
        ...
        # 末端所有非 None 值都等于 last_val → 厂字
        return all(v == last_val for v in tail_valid)
    except Exception:
        return False
...
def compute_remote_validation_score(
    ...
    zero_years = sum(
        ...
        if abs(turnover_list[i]) < 0.0001 and abs(returns_list[i]) < 0.0001
    )
    if zero_years / total_years > 0.50:
        issues.append("DEAD_ALPHA_RISK")
        score -= 40

    # ── 近2年有 Sharpe == 0 ─────────────────────────────────────────────── #
    recent_sharpes = sharpe_list[:2]
    if any(abs(s) < 1e-9 for s in recent_sharpes):
    ...
    if pos_years / total_years < 0.60:
        issues.append("unstable_returns")
        score -= 20

    # ── L2Y Sharpe vs IS Sharpe 对比 ─────────────────────────────────────── #
    if is_sharpe > 0 and l2y_sharpe < is_sharpe * 0.60:
