...
def generate_variants_for_plan(plan: OptimizationPlan, max_variants: int | None = 30) -> list[AlphaVariant]:
    ...
    for mode in plan.suggested_modes:
        for expression, reason in _expressions_for_mode(plan.expression, mode):
...
def generate_variants_for_alpha_id(alpha_id: str, max_variants: int = 30) -> tuple[OptimizationPlan | None, list[AlphaVariant]]:
    ...
    return plan, generate_variants_for_plan(plan, max_variants=max_variants)
