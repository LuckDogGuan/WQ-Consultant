...
| 类别 | 模块名称 | 来源文章 | 核心功能说明 |
|:---|:---|:---|:---|
| **Pruning** (剪枝) | [expression_pruner.py](./code/pruning/expression_pruner.py) | [构建因子...剪枝...](./unverified/构建因子表达式解析器对数据进行高效剪枝-prune%20函数新方法实现代码优化.md) | 基于表达式的语法结构对因子进行去重剪枝，同一结构中仅保留 Sharpe/Fitness 最高的因子。 |
| **Pruning** (剪枝) | [correlation_pruner.py](./code/pruning/correlation_pruner.py) | [结合自相关做剪枝...](./unverified/结合自相关做剪枝，提高回测效率代码优化.md) | 基于日收益率 Pearson 相关性矩阵进行贪心剪枝（保留相关度低且综合得分最高的因子组合）。 |
| **Optimization** (优化) | [robust_sharpe_optimizer.py](./code/optimization/robust_sharpe_optimizer.py) | [[工具分享]一键 Robust Sharpe...](./unverified/[工具分享]一键%20%20Robust%20Sharpe优化辅助.md) | 针对给定的 Alpha 进行三阶段多维度参数网格寻优（中性化 -> Decay/Truncation -> 表达式变体）。 |
| **Analysis** (分析) | [super_alpha_correlation.py](./code/analysis/super_alpha_correlation.py) | [【新人向-SA的本地计算SC】...](./unverified/【新人向-SA的本地计算SC】复制即用！跟上后一篇PC检测代码！代码优化.md) | 并行下载并批量计算目标 SuperAlpha 与当前 OS 阶段所有因子间的最大相关性，对低相关因子打标。 |
| **Analysis** (分析) | [fast_pnl_downloader.py](./code/analysis/fast_pnl_downloader.py) | [[另辟蹊径]不限额快速获取Pnl](./unverified/[另辟蹊径]不限额快速获取Pnl.md) | 绕过 WQ 每小时 2000 次的 standard PnL API 限额，利用比赛 before-and-after-performance 接口快速获取因子时序数据。 |
...
* **来源文件**: [构建因子...剪枝...](./unverified/构建因子表达式解析器对数据进行高效剪枝-prune%20函数新方法实现代码优化.md)
* **是否有用**: **非常有用**。
...
* **来源文件**: [结合自相关做剪枝...](./unverified/结合自相关做剪枝，提高回测效率代码优化.md)
* **是否有用**: **非常有用**。
...
* **来源文件**: [[工具分享]一键 Robust Sharpe...](./unverified/[工具分享]一键%20%20Robust%20Sharpe优化辅助.md)
* **是否有用**: **非常有用**。
...
* **来源文件**: [【新人向-SA的本地计算SC】...](./unverified/【新人向-SA的本地计算SC】复制即用！跟上后一篇PC检测代码！代码优化.md)
* **是否有用**: **非常有价值的工具**。
...
* **来源文件**: [[另辟蹊径]不限额快速获取Pnl](./unverified/[另辟蹊径]不限额快速获取Pnl.md)
* **是否有用**: **非常实用（API 级骚操作）**。
* **测试结果**: **语法与编译通过**。
...
### 6. [【wqapp优化】找灵感提示词优化经验分享.md](./unverified/【wqapp优化】找灵感提示词优化经验分享.md)
...
### 7. [VF0.98+Expert直升Grandmaster我是怎么做的.md](./unverified/VF0.98+Expert直升Grandmaster我是怎么做的.md)
...
### 8. [优化Sub-universe Sharpe的一个方法经验分享.md](./unverified/优化Sub-universe%20Sharpe的一个方法经验分享.md)
...
### 9. [利用mcp 和优化alpha工作流 成功优化GLB alpha的记录.md](./unverified/利用mcp%20和优化alpha工作流%20成功优化GLB%20alpha的记录.md)
...
### 11. [【缘分一座桥优化】甫子君优化工程1代码优化.md](./unverified/【缘分一座桥优化】甫子君优化工程1代码优化.md)
