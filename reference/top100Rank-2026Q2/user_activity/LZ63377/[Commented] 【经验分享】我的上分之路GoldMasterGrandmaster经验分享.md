...
- **链接**: [Commented] 【经验分享】我的上分之路GoldMasterGrandmaster经验分享.md
- **作者**: HP65370
- **发布时间/热度**: 3个月前, 得票: 98
...
![图片](images/img_34be539756.png)

![图片](images/img_11507ab708.png)

![图片](images/img_b3ecef16e9.png)
...
很好的经验分分享，希望我这赛季也能像佬一样gold->master->gm 
====================================================================================
==================纸上得来终觉浅，绝知此事要躬行======================================
...
感谢大佬分享经验，佬的combine太稳了，六维也很强，请教下佬的PPA tag的筛选和设置有什么好的标准么？
...
感谢大佬分享，我感觉我的六维这季度是没救了，我打算下季度再冲expert了
...
感谢GM大佬分享的工作流，确实感觉选好字段很重要，要不就把一天的回测都浪费在没信号的地方了。相关性也是挺让人苦恼的，我在想能不能通过聚类算法在分阶段过程中把相关性高的分类，选里面最好的跑
...
### 评论 #14 (作者: YH25030, 时间: 3个月前)

感谢大佬分享！学到了用非线性变换操作符tanh,sqrt,log,signed_power,arc_cos,exp。说实话，这些运算符我基本不用，以后要做尝试。还有我一般到赛季最后一个月点完塔，就很拉胯，提交因子减少。所以VF总是忽上忽下，季度奖也是保底。
...
这篇  **Gold → Master → GM 的完整上分路线** ，是我近期看过 **最系统、最能直接抄作业** 的新人成神攻略！

全程 **可落地、有数据、有流程、有避坑** ，看完直接少走 3 个月弯路。

### 一、最核心的结论（普通人也能复刻）

- 7 月入职， **3 个月 Gold→Master，再 3 个月 Master→GM**
- Combine 最高干到  **3.02** ，六维拉满
- 不靠玄学， **纯流程化、标准化出货**

### 二、Gold → Master 关键打法

1. **起步只做 1–2 个地区** （EUR/USA 最友好）
2. **只用传统一阶、二阶，不乱改模板**
3. **核心标准：Margin 必须达标，Turnover 不爆炸**
4. 少做双字段， **优先点塔 + 保数量**

### 三、Master → GM 的超级工作流（全文最值钱）

#### 1. 提前过滤字段（效率提升巨大）

- 按塔 / Region/Model 批量回测
- 把 **缺数据、全是 0 的字段拉黑**
- 用 API 自动判断，不再浪费回测次数

#### 2. 模板流极简优化（降低探索度 = 提高出货率）

- 固定时间窗口、固定操作符
- `group_op(ts_op(x, day), group)`  全部写死
- 目标： **少次数、快出基础信号**

#### 3. 字段内择优 + 去相关

- 同字段只留 **Sharpe、Fitness、Margin 最好的一个**
- 相关性 >0.5 直接丢掉
- 避免 “一个字段交一堆，互相卡死”
...
- 批量遍历：中性化、时间参数、非线性变换
- `tanh / sqrt / log / signed_power`  轮着上
- 自动入库→自动回测→自动筛选

#### 5. 回测资源分配（每天 5000 次怎么用）

- **多塔同时跑** ，不吊死在一个塔
- 点亮就换塔
- 最后用 Model/PV 这类好出的塔补数量

### 四、新人最容易踩的巨坑

- **最大连续回测天数断了 → 直接影响六维 & 定级**
- 提交量 **月末暴跌 → VF 暴跌、Combine 微跌**
- 同字段高相关 → 交再多也不涨分

### 五、一句话总结

> **点塔、六维、Combine 三管齐下，**
> **在有限的回测里，量产高质量低相关 Alpha。**

这篇属于 **存下来每周看一遍** 的顶级干货。

如果你正在 Gold/Expert/Master 卡关，照着这个流程复刻，下个季度你也能冲 GM。
...
### 评论 #23 (作者: SJ65808, 时间: 3个月前)

感谢大佬分享，每位GM都有其独特的挖因子方式，值得学习
...
感谢大佬无私分享自己的挖掘流程，对我很有帮助！

还想请教一下，在这个流程中应该在哪个阶段处理pc呢？是套完模板直接剔除高pc的alpha,还是在初加工或细加工后再剔除，会在最后手搓阶段再尝试一下吗？ 最近被高pc折磨得体无完肤。。。

另外大佬是如何使用ai在这个流程中发挥作用的呢？
...
### 评论 #36 (作者: HP65370, 时间: 2个月前)

[LL81909](/hc/en-us/profiles/31550055823511-LL81909)  我之前一直是采用单字段回测，因为多字段的组合方式太多了，需要耗费大量回测，另外选择哪些多字段来回测是一个问题，不过我在aiac比赛中尝试利用llm来选择双字段来组合，效果比较一般，可能后面还会继续探索，双字段的prod普遍要低一点。在字段初筛时，我中性化一般采用SUBINDUSTRY,比较快，或者RAM。

---

### 评论 #37 (作者: MG88592, 时间: 1个月前)
