# Combining Signals Without Overcomplicating Alphas

- **链接**: [Commented] Combining Signals Without Overcomplicating Alphas.md
- **作者**: JM47610
- **发布时间/热度**: 2个月前, 得票: 15

## 帖子正文

I’ve noticed adding more signals doesn’t always improve an alpha, but it often adds noise. Now I focus on my idea, combine only complementary signals, and keep things simple. Since simple alphas tend to be more stable and reliable.

How do you keep yours from getting overcomplicated?

---

## 讨论与评论 (7)

### 评论 #1 (作者: JO96892, 时间: 2个月前)

- Keeping alphas simple and robust while prioritizing OS robustness, not IS optimization.

---

### 评论 #2 (作者: PN39025, 时间: 2个月前)

It will have two sides, you know; some signals are quite low and need to be combined, while others don't.

---

### 评论 #3 (作者: RM79380, 时间: 2个月前)

Nice observation. It all narrows down to simplicity.

---

### 评论 #4 (作者: CM46415, 时间: 2个月前)

Good observations!

---

### 评论 #5 (作者: BO34089, 时间: 1个月前)

Good insight

---

### 评论 #6 (作者: VK29840, 时间: 1个月前)

that is very true

---

### 评论 #7 (作者: KU30147, 时间: 1个月前)

Try preserving a clear hypothesis for every signal added. If a feature doesn’t improve robustness, diversification, or out-of-sample behavior consistently, it’s usually complexity masquerading as improvement.

---

