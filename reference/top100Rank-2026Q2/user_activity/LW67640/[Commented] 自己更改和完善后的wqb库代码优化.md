...
- **链接**: [Commented] 自己更改和完善后的wqb库代码优化.md
- **作者**: YS95353
- **发布时间/热度**: 1年前, 得票: 21
...
        count_for_sending_email_wrapper,
        *args,
...
### 评论 #1 (作者: XW66626, 时间: 1年前)

大佬太强了

---

### 评论 #2 (作者: YP44571, 时间: 1年前)
...
### 评论 #3 (作者: YS95353, 时间: 1年前)

[YP44571](/hc/en-us/profiles/29178016678039-YP44571) 是的，这是我自定义的发邮件代码，用两个邮箱，一个作为自动发送者，另一个作为接收者，在绑定的微信上就能收到通知
...
SENDER_PASSWORD =  "yyy"  # QQ邮箱授权码

def send_email(subject:str, content:str=" ", receiver_email= [aaa@qq.com](mailto:aaa@qq.com) '):
...
### 评论 #4 (作者: YS95353, 时间: 1年前)

[XW66626](/hc/en-us/profiles/30244802421399-XW66626)

谢谢鼓励，还有很多需要提升的，一起进步，欢迎多交流🎇

---

### 评论 #5 (作者: ZR63005, 时间: 1年前)

感谢楼主无私的分享。

这里有个提议，或许楼主可以考虑去github提PR，这样未来wqb库直接就能够变成你修改的样子，拥有更多功能，从而方便更多人。

---

### 评论 #6 (作者: BW14163, 时间: 1年前)

感谢楼主分享，请问，使用该套代码进行回测，稳定性如何，卡住和api 限速的事件发生频率如何

---

### 评论 #7 (作者: YS95353, 时间: 1年前)

[ZR63005](/hc/en-us/profiles/28984775883543-ZR63005)

感谢认可，这套代码我使用了两个多月，最近也加上了super alpha的回测扩展，但最近太忙了没什么时间继续优化，日后有空做得更好一点可以考虑整套分享。自己总是觉得还不够完美😂

---

### 评论 #8 (作者: YS95353, 时间: 1年前)

[BW14163](/hc/en-us/profiles/28900537669399-BW14163)

我回测稳定性还不错，只要代码没有语法错误，基本能跑完不遗漏。回测基本是3*5=15次重试机会。

贴两张图看看，这是服务器不给力的时候，基本上有问题都捕获到并重试了。

![图片](images/img_8f1d4ce2e2.png)

![图片](images/img_c35a9f20b8.png)

---

### 评论 #9 (作者: LW67640, 时间: 1年前)
...
```
status_code: 429    reason: Too Many Requests    url: https://api.worldquantbrain.com/authentication    elapsed: 0:00:00.243682    headers: {'Date': 'Sun, 15 Jun 2025 06:45:33 GMT', 'Content-Type': 'application/json; charset=utf-8', 'Content-Length': '41', 'Connection': 'keep-alive', 'X-RateLimit-Remaining-Minute': '0', 'X-RateLimit-Limit-Minute': '5', 'RateLimit-Remaining': '0', 'RateLimit-Reset': '27', 'Retry-After': '27', 'RateLimit-Limit': '5', 'vary': 'Origin', 'Access-Control-Allow-Origin': 'https://platform.worldquantbrain.com', 'Access-Control-Allow-Credentials': 'true', 'Access-Control-Expose-Headers': 'Location,Retry-After', 'Strict-Transport-Security': 'max-age=31536000; includeSubDomains'}    text: {  "message":"API rate limit exceeded"
```

---

### 评论 #10 (作者: YS95353, 时间: 1年前)

[LW67640](/hc/en-us/profiles/28067010930967-LW67640)

您好！感谢认可。

这样的错误是正常的。大概是服务器那边要求重新登录认证，wqb库已经自动处理了这些事情，这里展示出来是为了让使用者便于查看，实际上不影响使用，只是会多几次充重试。风波过去之后还是会正常回测的。

---

### 评论 #11 (作者: YS95353, 时间: 1年前)

[LW67640](/hc/en-us/profiles/28067010930967-LW67640)

如果觉得这个输出太杂了，也可以在代码里定位到相应位置，注释掉headers、elapsed、reason等行，这样看着也舒服点。当然也可以直接省略输出。我是觉得有啥问题都能看到才放心，所以保留了输出。

---

### 评论 #12 (作者: SP75169, 时间: 1年前)

感谢楼主分享，请问这个错误应该如何解决，测试了直接把async def retry(）方法的alpha_id参数删除不可行，根据GPT的建议加入 kwargs.setdefault('params', {})['alpha_id'] = alpha_id  # 添加为 query 参数也未解决

```
TypeError: Session.request() got an unexpected keyword argument 'alpha_id'PS C:\Users\user\AppData\Local\Programs\Python\Python312\Lib\site-packages\wqb> & C:/Users/user/AppData/Local/Programs/Python/Python312/python.exe c:/Users/user/AppData/Local/Programs/Python/Python312/Lib/site-packages/wqb/simulate/test.pylen(alphas) = 14Traceback (most recent call last):  File "c:\Users\user\AppData\Local\Programs\Python\Python312\Lib\site-packages\wqb\simulate\test.py", line 67, in <module>    asyncio.run(main())  File "C:\Users\user\AppData\Local\Programs\Python\Python312\Lib\asyncio\runners.py", line 194, in run    return runner.run(main)           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^  File "C:\Users\user\AppData\Local\Programs\Python\Python312\Lib\site-packages\wqb\auto_auth_session.py", line 123, in request    resp = super().request(method, url, *args, **kwargs)           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^TypeError: Session.request() got an unexpected keyword argument 'alpha_id'
```

---

### 评论 #13 (作者: LH45966, 时间: 10个月前)

以下是我的代码，我用wqb每天只能跑四五千个，感觉远不如大家的两三万，求大佬解答帮助

```
 async def simulate_alphas_batch(self, alphas: List[Dict[str, Any]]) -> List[Dict[str, Any]]:        """        并发模拟Alpha批次，直接调用wqb.concurrent_simulate        """        if not alphas:            return []        unique_alphas = self.filter_duplicate_alphas(alphas)        if not unique_alphas:            self.logger.warning("所有Alpha表达式都重复，跳过本批次")            return []        self.logger.info(f"开始使用 wqb 并发模拟 {len(unique_alphas)} 个 Alpha (最大并发: {self.max_concurrent})...")        # 直接调用 wqb 库的并发模拟函数，它会处理所有并发、等待和重试        results = await self.wqbs.concurrent_simulate(            unique_alphas,            self.max_concurrent,            return_exceptions=True,  # 捕获异常而不是让程序崩溃            log="批量模拟",            tags="auto_simulated"        )        # 处理返回的结果        successful_simulations = []        for i, res in enumerate(results):            original_alpha = unique_alphas[i]            if isinstance(res, Exception):                self.failed_count += 1                self.logger.error(f"模拟 Alpha 时遇到异常: {res}")                self.record_failed_alpha(original_alpha, reason=f"异常: {str(res)[:100]}")            elif res and res.status_code < 400:                try:                    sim_data = res.json()                    if sim_data.get("status") == "COMPLETE":                        self.simulated_count += 1                        successful_simulations.append({'alpha': original_alpha, 'response_data': sim_data})                    else:                        self.failed_count += 1                        self.logger.warning(f"Alpha 回测状态未完成: {sim_data.get('status')}")                        self.record_failed_alpha(original_alpha, reason=f"回测状态未完成: {sim_data.get('status')}")                except Exception as e:                    self.failed_count += 1                    self.logger.error(f"解析成功响应时出错: {e}")                    self.record_failed_alpha(original_alpha, reason=f"解析响应失败: {e}")            else:                self.failed_count += 1                status = res.status_code if res else "No response"                self.logger.error(f"Alpha 回测失败，状态码: {status}")                self.record_failed_alpha(original_alpha, reason=f"回测失败，状态码: {status}")        self.logger.info(f"批次处理完成 - 成功: {len(successful_simulations)}, 失败: {self.failed_count}")        return successful_simulations
```
