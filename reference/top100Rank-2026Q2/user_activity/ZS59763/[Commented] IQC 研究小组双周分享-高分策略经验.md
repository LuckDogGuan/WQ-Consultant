...
- **链接**: [Commented] IQC 研究小组双周分享-高分策略经验.md
- **作者**: OL33779
- **发布时间/热度**: 1年前, 得票: 18
...
可以⽤double sorting来分别看解决⽅案.⽐如说图中的案例part，对于sharpe>1.4（⽐1.25的标准更⼤，容错⾜够）但fitness~（0.9，1）的alpha，我们可以通过调整decay来降频从⽽使得fitness满⾜要求，因为Fitness = Sharpe * Sqrt( Abs( Returns ) / Max( Turnover, 0.125 ) ) ，要么提⾼夏普要么降低turnover，⽽夏普不好提⾼，就只能降频了，decay不⼤时是有可能提⾼sharpe的（过滤低频波动），⽽对于sharpe>1.4的alpha，我们也可以接受牺牲部分的sharpe来提⾼fitness。 ![图片](images/img_d0cee98004.png) 
...
@ZS59763 谢谢反馈，学习了。（1）关于三阶模版，我昨晚也提到了该模版对数据的微小波动不太敏感，比如两个本身相关性高的data通过一阶其实不能产生较大的区分度，这也是后续对数据缩放的出发点。昨晚WL老师点评到三阶模板不是单模版而是处理思路，这样理解或许更好。（2）关于三阶的择时，我不太喜欢相同的进入和退出条件，因为这种操作会减少降低相关性的机会。（3）sub-sharpe确实也还需要积累，我目前总结了几个方法，能提交但是最终都不在高分的队列中，也希望有更好的方法

@LL87164 （1）是的（2）是IQC比赛中is加分多的alpha总结的特征，只能算是样本内的最优化，样本外表现还待验证。不过低相关是客观存在的（如下图，其中分数变化是每次提交我的记录），给大家一些参考。 ![图片](images/img_a3371b43f7.png)
